...
def chunk_split(map_file, output, chunk_size, chrms='', chunk=''):
    ...
    for df in pd.read_csv(map_file, sep='\\s+', engine='python', error_bad_lines=False, iterator=True, chunksize=chunk_size/100, names=["CHROM", "POS"]):
        ...
        for idx in range(len(list(chrm_datas))):
            ...
            data.append([chrm, d["POS"].min(), d["POS"].max()])
    datas = {}
    out = open(output, 'w')
    for dat in data:
        chrm = dat[0]
    ...
    for chrm in myPos:
        for pos in myPos[chrm]:
            ...
            out.writelines(
                ','.join([str(chrm), str(start_), str(end_)]) + '\\n')
    out.close()
...
if __name__ == '__main__':
    ...
    os.sync()
    time.sleep(2)
    print(f"chunks.txt written with {sum(1 for _ in open(outputFile))} lines")
